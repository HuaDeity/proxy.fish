function unset_proxy --description "Unset a specific proxy for a given plugin and type"
    ...
    if not contains $proxy_type $FISH_PROXY_TYPES
        echo "proxy.fish: Warning: Proxy type '$proxy_type' is not in FISH_PROXY_TYPES, but attempting to unset for $plugin_name." >&2
    end
    ...
    if functions -q $unsetter_func
        ...
        echo "proxy.fish: Unset $proxy_type proxy for $plugin_name."
    else
        echo "proxy.fish: Error: Unsetter function '$unsetter_func' not found for plugin '$plugin_name'." >&2
        return 1
    end
end
