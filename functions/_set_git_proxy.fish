        ...
        case http https
            ...
            if type -q git # Check if git command is available
                if test -n "$proxy_value"
                    ...
                    # echo "fish-proxy: (git) Set $git_config_key to $proxy_value"
                else
                    ...
                    # echo "fish-proxy: (git) Unset $git_config_key"
                end
            else
                # echo "fish-proxy: (git) 'git' command not found. Cannot configure Git proxy." >&2
            end
        case "*"
            # For other types like 'all' or 'no', Git doesn't have a direct equivalent.
            # Silently ignore or log if necessary.
            # echo "fish-proxy: (git) Proxy type '$proxy_type' not applicable for Git configuration."
            return 0
    end
end
